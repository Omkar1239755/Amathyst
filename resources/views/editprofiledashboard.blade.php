@extends('template.layout.app') @section('content') <style>
    .selected {
...
<style>
    .photo-upload-box {
        ...
        cursor: pointer;
        width: 100%;
        height: 300px; /* Or your specific height */
  ...
  <div class="row w-100 m-0">
    <main class="col-md-10 offset-md-2 px-4">
            <div id="saveMessage" class="mb-1"></div>
                    ...
                    <div class="d-flex align-items-center companionprofiledit-content flex-wrap">
                        <div class="d-flex align-items-center me-3"> <a href="{{route('dashboard')}}" class="btn btnarrow-link me-2"> <i class="fas fa-arrow-left"></i> </a>
                            ...
                            <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab" tabindex="0">
                                <div class="profile-card">
                                    ...
                                    <div class="mb-4 d-flex align-items-center">
                                        <div class="profile-photo me-3"> <img id="profilePreview" src="{{asset('uploads/profilecompanion/'.Auth::User()->profile_picture)}}" class="imgprofile-prev rounded-circle" value=width="20"> </div>
                                    ...
                                    <div class="row mb-3">
                                            ...
                                            <div class="mb-3"> <label class="form-label nameform">Birthday:</label>
                                                <div class="row g-2">
                                                    <div class="col-4"> <select id="birthDay" name="day" class="form-select custom-select">
                                                        ...
                                                        </select> </div> @php $months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; $selectedMonthName = ucfirst(strtolower(old('month', $user->month ?? ''))); $selectedMonthIndex = array_search($selectedMonthName, $months); @endphp <div class="col-4"> <select id="birthMonth" name="month" class="form-select">
                                        ...
                                        <div class="mb-4"> <label class="form-label">Bio</label> <textarea class="form-control formfield" name="bio" rows="3">{{ old('bio', $user->bio) }}</textarea> </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <!-- Interests Section -->
                                                <div class="section-box">
                                                    ...
                                                    <ul id="model-interest" class="tags list-unstyled d-flex flex-wrap gap-2 interest-tags"> @foreach($hobbies as $hobbie) <li class="tag-item"> <span><img src="{{ asset('uploads/hobbies/'.$hobbie->image) }}" width="20"></span> {{ ucfirst($hobbie->hobbie) }} <span class="remove-tag" onclick="removeTag('{{ $hobbie->id }}')"><i class="fas fa-times"></i></span> </li> @endforeach </ul> <!-- Hidden Input for Form -->
                                                    
                                                    <input type="hidden" name="selected_hobbies" id="selectedHobbiesInput" value="{{ $user->hobbie }}">
                                                
                                                </div>
                                                @php 
                                                $selectedHobbyIds = explode(',', $user->hobbie ?? '');
                                                @endphp
                                                <!-- Interests Modal -->
                                                <div class="modal fade" id="interestModal" tabindex="-1" aria-labelledby="interestModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                                        <div class="modal-content additional-model">
                                                            ...
                                                            <div class="modal-footer justify-content-center border-0"> <button type="button" class="btn btn-outline-secondary clearall-btn" onclick="clearhobbie()">Clear All </button> 
                                                            <button type="button" class="btn btn-primary confirmm-selectbtn" onclick="confirmSelection()" >Confirm</button> </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            </div> <!-- Hidden Input for Form --> 
                                            <input type="hidden" name="additional_hobbie" id="additionalHobbieInput" value="{{ $user->additional_hobbie }}"> 
                                            @php 
                                            $additionalHobbyIds = explode(',', $user->additional_hobbie ?? '');
                                            @endphp
                                            ...
                                            <div class="modal fade" id="additionalHobbyModal" tabindex="-1" aria-labelledby="additionalHobbyModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered modal-lg">
                                                    <div class="modal-content additional-model">
                                                        ...
                                                        <div class="modal-footer mt-3 justify-content-center border-0"> 
                                                        <button type="button" class="clearall-btn" onclick="clearAlladditional()">Clear All</button> 
                                                        <button type="button" class="confirmm-selectbtn" onclick="confirmAdditionalHobbySelection()">Confirm</button> </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <input type="hidden" name="traits" id="personal_traits" value="{{ $user->personal_trait }}"> @php $personaltraitIds = explode(',', $user->personal_trait ?? ''); @endphp
                                            ...
                                            <div class="modal fade" id="traitModal" tabindex="-1" aria-labelledby="traitModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered modal-lg">
                                                    <div class="modal-content additional-model">
                                                        ...
                                                        <div class="modal-footer mt-3 justify-content-center border-0"> <button type="button" class="clearall-btn" id="clear-traits-btn" onclick="clearAllTraits()">Clear All</button> <button type="button" class="confirmm-selectbtn" id="confirm-traits-btn" onclick="confirmTraitSelection()">Confirm</button> </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <!--Activity-->
                                        ...
                                        <div class="modal fade" id="activityModal" tabindex="-1" aria-labelledby="activityModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                <div class="modal-content additional-model">
                                                    ...
                                                    <div class="modal-footer justify-content-center border-0"> <button type="button" class="clearall-btn" onclick="clearAllActivities()">Clear All</button> <button type="button" class="confirmm-selectbtn" onclick="confirmActivitySelection()">Confirm</button> </div>
                                                </div>
                                            </div>
                                        </div>
 ...
 <!--***********************************************Images Upload ******************************************************************************************-->
       <form id="imageUploadForm" action="{{ route('images-companion') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab" tabindex="0">
                        <div class="card photoss">
                            <div class="your-photos">
                                <div class="row g-4">
                                    @foreach($images as $index => $image)
                                    <div class="col-md-3 position-relative">
                                        <!-- File input (hidden) -->
                                        <input type="file" name="images[{{ $index }}]" id="fileInput{{ $index }}" hidden onchange="previewImage(event, {{ $index }})" />
                                        <input type="hidden" name="existing_images[{{ $index }}]" value="{{ $image->images }}">
                                        <!-- Upload box -->
                                        <div class="photo-upload-box" id="customUpload{{ $index }}" onclick="document.getElementById('fileInput{{ $index }}').click();">
                                            <img id="preview{{ $index }}" src="{{ asset($image->images) }}" class="preview-imageprofile mt-2" style="width: 100%; height: 100%; object-fit: cover;" />
                                            <div class="upload-overlay"> <i class="fas fa-plus fa-2x"></i><br /> </div>
                                        </div>
                                        <button type="button" class="btn-danger remove-btn d-none" id="removeBtn{{ $index }}"> <i class="fas fa-times"></i> </button>
                                    </div>
                                    @endforeach
                                    <div class="text-start">
                                        <button type="submit" class="btn-savechnge mt-3" id="upload_galleryyyy">Save the Change</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<!--scripts from backend to store the data of hobbie traits additional hobbie  -->
<script>
...
<script>
    ...
    function renderTraitTags() {
        ...
        selectedTraits.forEach(function (trait) {
            $list.append(`
        <li class="tag-item d-flex align-items-center gap-1 bg-light px-2 py-1">
          <span><img src="${trait.image}" width="20" /></span> ${trait.name}
...
<script>
    ...
    function renderActivityTags() {
        ...
        selectedActivities.forEach(function (activity) {
            $list.append(`
                <li class="tag-item d-flex align-items-center gap-1 bg-light px-2 py-1">
                    <span><img src="${activity.image}" width="20" /></span> ${activity.name}
...
<!--script added to store the data in data base via ajax-->
<script>
    $(document).ready(function () {
...
<!--script added to view the profile picture-->
<script>
    document.addEventListener("DOMContentLoaded", function () {
