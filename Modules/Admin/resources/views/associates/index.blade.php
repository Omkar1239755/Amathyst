                			...
                			<h3 class="card-title">Associate</h3>
                                       ...
                                       @foreach ($associates as $index => $associate)
                                            <tr>
                                                ...
                                                <!--<td>{{ ($associate->user_name)?$associate->user_name:'-' }}</td>-->
                                               
                                               <!-- <td>-->
                                               <!--     <label class="switch">-->
                                               <!--         <input type="checkbox" data-id="{{$associate->id}}" class="toggle-class" {{ $associate->status ? 'checked' : '' }}>-->
                                               <!--         <span class="slider round"></span>-->
                                               <!--     </label>-->
                                               <!--</td>-->
                                               <td class="table_action text-center" style="white-space: nowrap;">
                                                   
                                                    <!--<form method="POST" action="{{ route('associate.delete', $associate->id) }}" style="display: inline;">-->
                                                    <!--    @csrf-->
                                                    <!--    @method('DELETE')-->
                                                    ...
                                                    <a href="{{ route('associate.view', $associate->id) }}" class="edit-btn" style="margin-left: 10px; color: green;">
                                                        <i class="fa fa-eye"></i>
                                                    </a>
                                                </td>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('associate.update-status') }}',
                    data: {
                        'status': status,
                        'user_id': user_id
                    },
